...
--drop table flota.poliza;

alter table flota.coche add constraint modelo_fk foreign key(id_modelo) references flota.modelo(id);
alter table flota.coche add constraint color_fk foreign key(id_color) references flota.color(id);
alter table flota.coche add constraint poliza_fk foreign key(id_poliza) references flota.poliza(id);
alter table flota.modelo add constraint modelo_fk foreign key(id_marca) references flota.marca(id);
alter table flota.marca add constraint grupo_fk foreign key(id_grupo) references flota.grupo(id);
alter table flota.revision add constraint coche_fk foreign key(id_coche) references flota.coche(id);
alter table flota.revision add constraint moneda_fk foreign key(id_moneda) references flota.moneda (id);
alter table flota.poliza add constraint aseguradora_fk foreign key(id_aseguradora) references flota.aseguradora(id);
...
--select * from flota.coches;

insert into flota.color (nombre) select color from flota.coches group by color order by color;

--select * from flota.color;

insert into flota.grupo(nombre) select grupo from flota.coches group by grupo order by grupo;

--select * from flota.grupo;

insert into flota.moneda (nombre) select moneda from flota.coches group by moneda order by moneda;

--select * from flota.moneda;

insert into flota.aseguradora (nombre) select aseguradora from flota.coches group by aseguradora order by aseguradora;

--select * from flota.aseguradora;

insert into flota.marca (nombre,id_grupo)
select 
	fl.marca,	
	g.id
from flota.coches fl
inner join flota.grupo g on fl.grupo = g.nombre
group by fl.marca, g.id
order by g.id, fl.marca;
...
select 
	...
	m.id
from flota.coches fl
inner join flota.marca m on fl.marca = m.nombre
group by fl.modelo, m.id
order by m.id, fl.modelo;

--select * from flota.modelo;

insert into flota.poliza (numero,fecha,id_aseguradora)
select
	fl.n_poliza,
	cast(fl.fecha_alta_seguro as date),	
	a.id
from flota.coches fl
inner join flota.aseguradora a on fl.aseguradora = a.nombre
group by fl.n_poliza, fl.fecha_alta_seguro,a.id
order by a.id, fl.fecha_alta_seguro, fl.n_poliza;

--select * from flota.poliza;

insert into flota.coche (matricula,id_modelo,fecha_compra,id_color,kms,id_poliza)
select
fl.matricula,
m.id,
cast(fl.fecha_compra as date),
c.id,
fl.kms_totales,
p.id
from flota.coches fl
inner join flota.modelo m on fl.modelo=m.nombre
inner join flota.color c on fl.color = c.nombre
inner join flota.poliza p on fl.n_poliza = p.numero
...
--select * from flota.coche;

insert into flota.revision(id_coche,km_revision,importe,id_moneda,fecha_revision)
select
c.id,
fl.kms_revision,
fl.importe_revision,
m.id,
cast(fl.fecha_revision as date)
from flota.coches fl
inner join flota.coche c on fl.matricula=c.matricula
inner join flota.moneda m on fl.moneda=m.nombre
group by 
c.id,
fl.kms_revision,
fl.importe_revision,
...
fa.nombre
from flota.coche fc
inner join flota.modelo fm
on fc.id_modelo = fm.id 
inner join flota.marca fmm
on fm.id_marca = fmm.id
inner join flota.grupo fg
on fmm.id_grupo = fg.id
inner join flota.color fco
on fc.id_color = fco.id
inner join flota.poliza fp
on fc.id_poliza = fp.id
inner join flota.aseguradora fa
on fp.id_aseguradora = fa.id
group by fm.nombre, 
fmm.nombre, 
fg.nombre,
fc.fecha_compra,
